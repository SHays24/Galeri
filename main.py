...
class MenuBar:
  ...
  def addSubmenu(self, menu, upmenu, oplist, command):
    if (command == "Year"):
      ...
      def find():
          fdr = ImageRef.findYears(self.win.ImageGrid.getImgs(self.win.ImageGrid), str(menu.menu_filter.year.option.get()))
          # fdr = self.win.ImageGrid.getImgs(self.win.ImageGrid)
          self.win.ImageGrid.ShowGrid(fdr)
    elif (command == "Month"):
      var = menu.menu_filter.month.option
      def find():
          fdr = ImageRef.findMonths(self.win.ImageGrid.getImgs(self.win.ImageGrid), str(menu.menu_filter.month.option.get()))
          # fdr = self.win.ImageGrid.getImgs(self.win.ImageGrid)
          self.win.ImageGrid.ShowGrid(fdr)

    upmenu.add_radiobutton(label="All", variable=var, value="All", command=lambda: find())
    print(oplist)
    for option in oplist:
      upmenu.add_radiobutton(label=option, variable=var, value=option, command=lambda: find())
      print(option)
...
if __name__ == '__main__':
  ...
  def hello1():
    print("hell1o")
  def command():
    app.menubar.addSubmenu(app.menubar, app.menubar.menu_filter.year, ImageRef.getYears(app.ImageGrid.getImgs(app.ImageGrid)), "Year")
    print(ImageRef.getYears(app.ImageGrid.getImgs(app.ImageGrid)))
  def command1():
    app.menubar.addSubmenu(app.menubar, app.menubar.menu_filter.month, ImageRef.getMonths(app.ImageGrid.getImgs(app.ImageGrid)), "Month")
